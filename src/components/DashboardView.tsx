...
export default function DashboardView({ state, onSyncAll, syncing, onNavigate }: DashboardViewProps) {
  const { profiles, settings, blocklists, watchlist } = state;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="stats-grid">
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-3">
            <span className="text-cyan-400 font-semibold">Real-time</span> device-level monitoring
          </p>
        </motion.div>
      </div>
                  ...
                  <div key={i} className="flex justify-between items-center p-3 bg-slate-950 border border-slate-800/80 rounded-xl">
                    <div className="space-y-0.5">
                      ...
                      <span className="text-[10px] text-slate-500">
                        Last Blocked: {new Date(bd.lastBlockedAt).toLocaleString()}
                      </span>
                    </div>
